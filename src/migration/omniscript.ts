...
export class OmniScriptMigrationTool extends BaseMigrationTool implements MigrationTool {
  ...
  getRecordName(record: string) {
    return (
      record[ISUSECASE2 ? OmniScriptMappings['Type__c'] : this.namespacePrefix + 'Type__c'] +
      '_' +
      record[ISUSECASE2 ? OmniScriptMappings['SubType__c'] : this.namespacePrefix + 'SubType__c'] +
      record[ISUSECASE2 ? OmniScriptMappings['Language__c'] : this.namespacePrefix + 'Language__c'] +
      '_' +
      record[ISUSECASE2 ? OmniScriptMappings['Version__c'] : this.namespacePrefix + 'Version__c']
    );
  }
    ...
    for (const omniscript of omniscripts) {
      ...
      try {
        ...
        const omniProcessType = omniscript[
          ISUSECASE2 ? OmniScriptMappings['IsProcedure__c'] : this.namespacePrefix + 'IsProcedure__c'
        ]
      ...
      if (omniAssessmentInfo.type === 'OmniScript') {
        const type = omniscript[
          ISUSECASE2 ? OmniScriptMappings['IsLwcEnabled__c'] : this.namespacePrefix + 'IsLwcEnabled__c'
        ]
          ? 'LWC'
          : 'Angular';
  ...
  private async processOmniScript(
    ...
    for (const elem of elements) {
      const type = elem[ISUSECASE2 ? OmniScriptMappings['Type__c'] : this.namespacePrefix + 'Type__c'];
      const elemName = `${elem['Name']}`;
      const propertySet = JSON.parse(
        elem[ISUSECASE2 ? OmniScriptMappings['PropertySet__c'] : this.namespacePrefix + 'PropertySet__c'] || '{}'
      );
    ...
    const existingType = omniscript[ISUSECASE2 ? OmniScriptMappings['Type__c'] : this.namespacePrefix + 'Type__c'];
    const existingTypeVal = new StringVal(existingType, 'type');
    const existingSubType =
      omniscript[ISUSECASE2 ? OmniScriptMappings['SubType__c'] : this.namespacePrefix + 'SubType__c'];
    const existingSubTypeVal = new StringVal(existingSubType, 'sub type');
    const omniScriptName = omniscript[ISUSECASE2 ? OmniScriptMappings['Name'] : this.namespacePrefix + 'Name'];
    ...
    const newLanguage = omniscript[
      ISUSECASE2 ? OmniScriptMappings['Language__c'] : this.namespacePrefix + 'Language__c'
    ]
      ? `${omniscript[ISUSECASE2 ? OmniScriptMappings['Language__c'] : this.namespacePrefix + 'Language__c']}`
      : '';
    ...
    const recordName =
      ...
      `${newSubType}` +
      (omniscript[ISUSECASE2 ? OmniScriptMappings['Language__c'] : this.namespacePrefix + 'Language__c']
        ? `_${omniscript[ISUSECASE2 ? OmniScriptMappings['Language__c'] : this.namespacePrefix + 'Language__c']}`
        : '') +
      `_${omniscript[ISUSECASE2 ? OmniScriptMappings['Version__c'] : this.namespacePrefix + 'Version__c']}`;
  ...
  async migrate(): Promise<MigrationResult[]> {
    ...
    let filteredOmniscripts = omniscripts.filter(
      (omniscript: any) =>
        typeof omniscript === 'object' && 'Name' in omniscript && omniscript.Name.includes('ABCTestOmniscript')
    );
    omniscripts = filteredOmniscripts;
    ...
    for (let omniscript of omniscripts) {
      ...
      const isOsActive =
        omniscript[ISUSECASE2 ? OmniScriptMappings['IsActive__c'] : `${this.namespacePrefix}IsActive__c`];
      ...
      omniscript[ISUSECASE2 ? OmniScriptMappings['IsActive__c'] : `${this.namespacePrefix}IsActive__c`] = false;
      ...
      if (
        omniscript[ISUSECASE2 ? OmniScriptMappings['IsProcedure__c'] : `${this.namespacePrefix}IsProcedure__c`] === true
      ) {
        // Check for missing mandatory fields for Integration Procedures
        const existingType = omniscript[ISUSECASE2 ? OmniScriptMappings['Type__c'] : this.namespacePrefix + 'Type__c'];
        const existingSubType =
        ...
        if (functionDefinitionMetadata.length > 0 && elements.length > 0) {
          for (let ipElement of elements) {
            if (
              ipElement[ISUSECASE2 ? OmniScriptMappings['PropertySet__c'] : `${this.namespacePrefix}PropertySet__c`] !=
              null
            ) {
              // Check for reserved keys while processing the PropertySet
              const propertySet = JSON.parse(
                ipElement[
                  ISUSECASE2 ? OmniScriptMappings['PropertySet__c'] : `${this.namespacePrefix}PropertySet__c`
                ] || '{}'
              );
              this.collectReservedKeys(propertySet, foundReservedKeys);
      ...
      let standardRecordId;
      // Here save Id and later append it
      if (ISUSECASE2) {
        mappedOmniScript = omniscript;
        // Remove the ID field
        standardRecordId = mappedOmniScript['Id'];
      ...
      mappedOmniScript[OmniScriptMappings.Type__c] = 'MyTypeABCForTesting';
      ...
      // Here see if type subtype has been updated then how to upload it inline
      ...
      if (osUploadResponse?.success) {
        ...
        // TP - Here the osuploadResponse should have Id

        try {
          // Upload All elements for each OmniScript__c record(i.e IP/OS)
          await this.uploadAllElements(osUploadResponse, elements);

          // Now need to understand the Omniscript Compiled Definition part of the record
          ...
          if (isOsActive) {
            ...
            const updateResult = await NetUtils.updateOne(
              this.connection,
              ISUSECASE2 ? OmniScriptMigrationTool.OMNIPROCESS_NAME : OmniScriptMigrationTool.OMNISCRIPT_NAME,
              recordId,
              osUploadResponse.id,
              {
                [ISUSECASE2 ? OmniScriptMappings.IsActive__c : `${this.namespacePrefix}IsActive__c`]: true,
              }
            );
  ...
  private async getOmniScriptCompiledDefinition(recordId: string): Promise<AnyJson[]> {
    ...
    const filters = new Map<string, any>();
    filters.set(ISUSECASE2 ? 'OmniProcessId' : this.namespacePrefix + 'OmniScriptId__c', recordId);
  ...
  private async uploadAllElements(
    ...
    do {
      ...
      for (let element of elements) {
        if (element[ISUSECASE2 ? ElementMappings['Level__c'] : `${this.namespacePrefix}Level__c`] === levelCount) {
          let elementId = element['Id'];
          let elementParentId =
            element[ISUSECASE2 ? ElementMappings['ParentElementId__c'] : `${this.namespacePrefix}ParentElementId__c`];
          if (
            !elementsUploadInfo.has(elementId) &&
            (!elementParentId || (elementParentId && elementsUploadInfo.has(elementParentId)))
          ) {
            tempElements.push(element); // Need to see why some of these conditions did not succedd for element of Level-1
          }
        }
      }
      ...
      if (tempElements.length === 0) {
        ...
        // Initialize the elementsUploadResponse
        ...
        if (!ISUSECASE2) {
          ...
          Logger.logVerbose('ABCD');
          Logger.logVerbose(JSON.stringify(elementsUploadResponse));
        }

        if (ISUSECASE2) {
          // elementsUploadResponse = await this.updateData(
          //  elementsTransformedData
          // )

          // Here will have a for loop to iterate and modify

          for (let elementRecord of elementsTransformedData.mappedRecords) {
            let savedId = elementRecord['Id'];
            let savedOmniProcessId = elementRecord['OmniProcessId'];
            ...
            elementRecord['OmniProcessId'] = savedOmniProcessId;
            Logger.logVerbose(JSON.stringify(response));

            elementsUploadResponse[savedId] = response;
          }
        }
  ...
  private async uploadAllOmniScriptDefinitions(
    ...
    // TODO - Need to see here if osDefinition is an array of elements, then which all ids we need to target
    if (ISUSECASE2) {
      for (let osDefinitionRecord of osDefinitionsData.mappedRecords) {
        let idSaved = osDefinitionRecord['Id'];
        let omniProcessIdSaved = osDefinitionRecord['OmniProcessId'];
        delete osDefinitionRecord['Id'];
        delete osDefinitionRecord['OmniProcessId'];

        Logger.logVerbose(omniProcessIdSaved);
        ...
        Logger.logVerbose(JSON.stringify(osUploadResponse));
      }
    }
  }
  ...
  private mapElementData(
    ...
    const elementType = mappedObject[ElementMappings.Type__c]; // NEED TO SEE THIS
